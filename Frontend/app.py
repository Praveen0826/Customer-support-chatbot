...
#     # Initialize session for messages and setup steps if not already present
#     if 'messages' not in session:
#         session['messages'] = []
#         session['setup_step'] = 0  # Step counter for setup questions
...
#             save_conversation(username, user_input, 'user')

#             if setup_step == 0:  # Name
#                 messages.append({'sender': 'bot', 'text': f"Nice to meet you, {user_input}!"})
#                 session['name'] = user_input
#             elif setup_step == 1:  # Product
#                 messages.append({'sender': 'bot', 'text': f"Got it, you're asking about {user_input}."})
#                 session['product'] = user_input
#             elif setup_step == 2:  # Order Number
#                 messages.append({'sender': 'bot', 'text': f"Thanks for providing order number {user_input}."})
#                 session['order_number'] = user_input
#             elif setup_step == 3:  # Issue Type
#                 messages.append({'sender': 'bot', 'text': f"Noted your issue type as {user_input}."})
#                 session['issue_type'] = user_input
#             elif setup_step == 4:  # Contact Method
#                 messages.append({'sender': 'bot', 'text': f"We'll use {user_input} if we need to reach you."})
#                 session['contact_method'] = user_input
#             elif setup_step == 5:  # Completion
#                 messages.append({'sender': 'bot', 'text': f"Thanks {session['name']}! Do you have any Other Questions  {session['product']} queries. Type 'quit' to exit the conversation."})

#             # Increment setup step
#             session['setup_step'] += 1
#             save_conversation(username, messages[-1]['text'], 'bot')
#             session['messages'] = messages
#             return redirect(url_for('chatbot'))

#         # Display setup question
#         if setup_step == 0:
#             messages.append({'sender': 'bot', 'text': "What is your user name?"})
#         elif setup_step == 1:
#             messages.append({'sender': 'bot', 'text': "What product are you inquiring about?"})
#         elif setup_step == 2:
#             messages.append({'sender': 'bot', 'text': "What is your order number (if applicable)?"})
#         elif setup_step == 3:
#             messages.append({'sender': 'bot', 'text': "What type of issue are you facing?\n1. Return | 2. Technical | 3. Delivery | 4. Other"})
#         elif setup_step == 4:
#             messages.append({'sender': 'bot', 'text': "How would you prefer to be contacted if needed (email/phone)?"})
#         elif setup_step == 5:
#             pass  # No additional message needed here

#         return render_template('chatbot.html', username=username, messages=messages)

#     # Normal chat interaction after setup
#     if request.method == 'POST':
#         user_input = request.form['message']
#         messages.append({'sender': 'user', 'text': user_input})
#         save_conversation(username, user_input, 'user')
...
#             bot_response = f"Goodbye {session['name']}!"
#         else:
#             # Use ML model to generate responses
#             processed_query = preprocess_text(user_input)
#             query_tfidf = tfidf.transform([processed_query])
#             intent_pred = svm.predict(query_tfidf)
#             intent = le.inverse_transform(intent_pred)[0]
#             bot_response = intent_responses.get(intent, "I'm not sure how to help with that. Please try rephrasing your question.")

#         messages.append({'sender': 'bot', 'text': bot_response})
#         save_conversation(username, bot_response, 'bot')
...
# # Conversations route to display all conversations
...
#     username = session['username']
#     all_conversations = get_conversations(username)
#     return render_template('conversations.html', username=username, conversations=all_conversations)

# # Logout route
# @app.route('/logout')
# def logout():
#     session.pop('username', None)
#     session.pop('messages', None)  # Clear chat history on logout
